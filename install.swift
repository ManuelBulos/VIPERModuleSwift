...
func moveTemplate(){
    ...
    do {
        if !fileManager.fileExists(atPath:"\(destinationPath)/\(templateName)"){
            ...
            printInConsole("✅  Template already exists. So it has been replaced succesfully 🎉. Enjoy 🙂")
        }
    }
    catch let error as NSError {
        printInConsole("❌  Ooops! Something went wrong ☹️ : \(error.localizedFailureReason)")
    }
}
